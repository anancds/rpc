...
void EvHttpServ::InitServer() {
  if (servAddr_ != "") {
    int64_t uAddr = inet_addr(servAddr_.c_str());
...
void EvHttpServ::SetTimeOut(int seconds) { evhttp_set_timeout(evHttp_, seconds); }

void EvHttpServ::SetAllowedMethod(HttpMethodsSet methods) { evhttp_set_allowed_methods(evHttp_, methods); }

void EvHttpServ::SetMaxHeaderSize(size_t num) { evhttp_set_max_headers_size(evHttp_, num); }

void EvHttpServ::SetMaxBodySize(size_t num) { evhttp_set_max_body_size(evHttp_, num); }

bool EvHttpServ::RegistHandler(std::string const &strUrl, handle_t *funcPtr) {
  if (!is_init) {
    EvHttpServ::InitServer();
  }
...
bool EvHttpServ::UnRegistHandler(std::string const &strUrl) { return (0 == evhttp_del_cb(evHttp_, strUrl.c_str())); }
